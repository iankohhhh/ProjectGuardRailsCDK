...
#---------------------------------------------------------REQUEST BODY CHECK----------------------------------------------------------
# Checks that tenant_id field in the query string parameter == JWT, if not return an Unauthorized message
...
#Rule 1: Check Schema against OpenAPI definition file
    ...
    if errors:
        ...
        update_ipset([request_source_ip])
...
#Rule 2: If no errors in rule 1, then proceed to check that every tenant id field in the response == tenant id field in JWT        
        ...
        for tid in list_of_tid:
            if int(tid) != tenant_id_jwt:
                ...
                "isBase64Encoded": False,
                "statusCode": 401,
                "body": json.dumps("Unauthorized. Response data includes items that does not belong to tenant" + request_source_ip)
                }
